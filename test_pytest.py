...
def test_update_education():
    """
    Update an existing educatin and check if the update was successful
    """
    example_education = {
        "course": "Engineering",
        "school": "NYU",
    ...
    update_response = client.put(f"/resume/education/{item_id}", json=updated_education)
    assert update_response.status_code == 200

    get_response = client.get(f"/resume/education/{item_id}")
    assert get_response.json["course"] == updated_education["course"]
    assert get_response.json["school"] == updated_education["school"]
    assert get_response.json["start_date"] == updated_education["start_date"]
    assert get_response.json["end_date"] == updated_education["end_date"]
    assert get_response.json["grade"] == updated_education["grade"]
